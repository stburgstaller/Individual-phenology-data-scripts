...
scaledMassGT <-
  function(x, y) { #"x = SVL (mm)","y = mass (g)"
    SMI.gt <- y * (60 / x) ^ 3 
    return(SMI.gt)}
...
# reduce data by svl cut-offs
#data_for_glm <- data_for_glm[data_for_glm$svl >= 55, ]
#data_for_glm <- data_for_glm[data_for_glm$svl >= 60, ]
...
#calculate the SMI using the function scaledMassGT() (see lines 17-20)
...
First_glmm <- glmmTMB(Day_of_the_year ~ gtSMI + svl + sex + (1|ID) + (1|year), 
                    family="gaussian", data=data_for_glm[data_for_glm$Arrival=="First",]) 
# model for last encounters
Last_glmm <- glmmTMB(Day_of_the_year ~ gtSMI + svl + sex + (1|ID) + (1|year),
                   family="gaussian", data=data_for_glm[data_for_glm$Arrival=="Last",])
...
tab_model(First_glmm,show.intercept = F, show.icc=F,show.stat =T,string.stat="z",
          pred.labels = c("Scaled mass index", "Snout vent length", "Sex (male)", "Sex (unsexed)"), file = "Table_1.doc")

tab_model(Last_glmm,show.intercept = F, show.icc=F,show.stat =T,string.stat="z",
          pred.labels = c("Scaled mass index", "Snout vent length", "Sex (male)", "Sex (unsexed)"), file = "Table_2.doc")
